  ...
  module SProduct
    class BackCalls
      ...
      def mount_total_labels
        ActivityRecord.where(data_id: @product_id, data_type: "Product", period: 1, type_record: 3).limit(15).map { |record| "#{record.date.mday}.#{record.date.mon}" }
      end

      def mount_total_series
        ActivityRecord.where(data_id: @product_id, data_type: "Product", period: 1, type_record: 3).limit(15).map { |record| record.time_range.total }
      end
      ...
      def mount_times_back_calls_stolb
        ...
        ActivityRecord.where(data_id: @product_id, data_type: "Product", period: 1, type_record: 3).limit(15).each do |record|
