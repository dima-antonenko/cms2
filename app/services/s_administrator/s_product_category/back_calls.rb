  ...
  module SProductCategory
    class BackCalls
      ...
      def mount_total_labels
        ActivityRecord.where(data_id: @product_category_id, data_type: "ProductCategory", period: 1, type_record: 2).limit(15).map { |record| "#{record.date.mday}.#{record.date.mon}" }
      end

      def mount_total_series
        ActivityRecord.where(data_id: @product_category_id, data_type: "ProductCategory", period: 1, type_record: 2).limit(15).map { |record| record.time_range.total }
      end
      ...
      def mount_times_back_calls_stolb
        returned_array = [0, 0, 0, 0, 0, 0]
        ActivityRecord.where(data_id: @product_category_id, data_type: "ProductCategory", period: 1, type_record: 2).limit(15).each do |record|
