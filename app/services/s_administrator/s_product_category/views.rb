  ...
  module SProductCategory
    class Views
      ...
      def mount_total_labels
        ActivityRecord.where(data_id: @product_category_id, data_type: "ProductCategory", period: 1, type_record: 1).limit(15).map { |record| "#{record.date.mday}.#{record.date.mon}" }
      end

      def mount_total_series
        ActivityRecord.where(data_id: @product_category_id, data_type: "ProductCategory", period: 1, type_record: 1).limit(15).map { |record| record.time_range.total }
      end
